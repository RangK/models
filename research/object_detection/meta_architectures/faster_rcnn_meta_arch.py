...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    def _loss_box_classifier(self,
        ...
        with tf.name_scope('BoxClassifierLoss'):
            ...
            print("max num_proposals : {}".format(self.max_num_proposals))

            proposal_boxes = tf.Print(proposal_boxes, [proposal_boxes], "proposal_boxes : ", summarize=100)
            ...
            # Proposal box의 수를 최소 1개는 가지도록 값을 수정
            # 1차원 matrix를 2차원으로 변경
            num_proposals_or_one = tf.to_float(
                tf.expand_dims(
                    tf.maximum(num_proposals, tf.ones_like(num_proposals)),
                    1
                )
            )
            ...
            if prediction_masks is not None:
                ...
                if not self._is_training:
                    ...
                    proposal_boxes = tf.Print(proposal_boxes, [proposal_boxes], "mask proposal_boxes : ", summarize=100)
    ...
    def _get_mask_proposal_boxes_and_classes(self, detection_boxes, num_detections,
                                             image_shape, groundtruth_boxlists,
                                             groundtruth_classes_with_background_list,
                                             groundtruth_weights_list):
        ...
        detection_boxes = tf.Print(detection_boxes, [detection_boxes], "detection_boxes : ", summarize=100)
        num_detections = tf.Print(num_detections, [num_detections], "num_detections : ", summarize=100)
        image_shape = tf.Print(image_shape, [image_shape], "image_shape : ", summarize=100)

        batch, max_num_detections, _ = detection_boxes.shape.as_list()
        proposal_boxes = tf.reshape(
            box_list_ops.to_absolute_coordinates(
                box_list.BoxList(
                    tf.reshape(detection_boxes, [-1, 4])
                ), image_shape[1], image_shape[2]
            ).get(), [batch, max_num_detections, 4]
        )
